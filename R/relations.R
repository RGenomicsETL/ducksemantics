...
ducksemantics_monarch_release_catalog <- function(releases, conn = NULL) {
  ...
  releases <- ducksemantics_require_data_frame(
    releases,
    "releases",
    c("release_id", "provider_id")
  )
  has_date <- "effective_date" %in% names(releases)
  has_ordinal <- "source_ordinal" %in% names(releases)
  if (!has_date && !has_ordinal) {
    stop(
      "`releases` must contain typed `effective_date` or numeric `source_ordinal`.",
      call. = FALSE
    )
  }
  if (!has_date) {
    releases$effective_date <- as.Date(
      rep(NA_real_, nrow(releases)),
      origin = "1970-01-01"
    )
  }
  if (!has_ordinal) {
    releases$source_ordinal <- rep(NA_real_, nrow(releases))
  }
  releases <- releases[,
    c("release_id", "provider_id", "effective_date", "source_ordinal"),
    drop = FALSE
  ]
  ...
  effective_date <- ducksemantics_typed_time(
  ...
  if (
    any(
      !is.na(releases$source_ordinal) &
        (is.na(source_ordinal) | !is.finite(source_ordinal))
    )
  ) {
    stop(
      "`releases$source_ordinal` must contain finite numbers or NA.",
      call. = FALSE
    )
  }
  if (any(is.na(effective_date) & is.na(source_ordinal))) {
    stop(
      "Every release must have `effective_date` or `source_ordinal`.",
      call. = FALSE
    )
  }
  if (anyDuplicated(releases[c("provider_id", "release_id")])) {
    stop(
      "`provider_id` and `release_id` must uniquely identify catalog rows.",
      call. = FALSE
    )
  }
...
#' @export
ducksemantics_monarch_import <- function(
  facts,
  releases,
  ...
  rows <- ducksemantics_monarch_normalize(
    ducksemantics_collect_relation(facts, conn, "facts"),
    relation
  )
  ...
  if (!is.null(conn)) {
    ...
    table <- ducksemantics_require_identifier(
      table %||% paste0("semantic_monarch_", relation),
      "table",
      TRUE
    )
    DBI::dbWriteTable(
      ...
      table,
      rows,
      overwrite = ducksemantics_require_flag(replace, "replace"),
      append = !replace
    )
...
#' @export
ducksemantics_monarch_query <- function(
  facts,
  releases,
  relation = c("gene_phenotype", "gene_disease", "disease_phenotype"),
  provider_id,
  ...
  out <- rows[
    rows$provider_id == provider_id &
      rows$release_id == selected$release_id[[1L]],
    ,
    drop = FALSE
  ]
  row.names(out) <- NULL
...
#' @export
ducksemantics_monarch_gene_disease_holdout_audit <- function(
  facts,
  releases,
  provider_id,
  train_release_id,
  ...
  train <- ducksemantics_monarch_query(
    facts,
    releases,
    "gene_disease",
    provider_id,
    release_id = train_release_id,
    conn = conn
  )
  holdout <- ducksemantics_monarch_query(
    facts,
    releases,
    "gene_disease",
    provider_id,
    release_id = holdout_release_id,
    conn = conn
  )
  train_key <- paste(
    train$gene_id,
...
ducksemantics_monarch_normalize <- function(facts, relation) {
  ...
  facts <- ducksemantics_require_data_frame(
    facts,
    "facts",
    c(roles, "release_id", "provider_id")
  )
  facts <- ducksemantics_add_missing(
    facts,
    c(
      predicate = if (identical(roles[[2L]], "phenotype_id")) {
  ...
  ducksemantics_validate_required_text(
    facts,
    c(roles, "predicate", "release_id", "provider_id"),
    "facts"
  )
  ducksemantics_validate_optional_text(
    facts,
    c("source_version", "attrs"),
    "facts"
  )
  unique(facts)
}
...
ducksemantics_monarch_validate_fact_catalog <- function(facts, catalog) {
  ...
  if (length(unknown)) {
    stop(
      "Monarch facts reference provider/release rows absent from `releases`.",
      call. = FALSE
    )
  }
  invisible(TRUE)
...
ducksemantics_monarch_select_release <- function(
  catalog,
  provider_id,
  release_id = NULL,
  as_of = NULL,
  ...
  if (sum(provided) != 1L) {
    stop(
      "Supply exactly one of `release_id`, typed `as_of`, or `as_of_ordinal`.",
      call. = FALSE
    )
  }
  candidates <- catalog[catalog$provider_id == provider_id, , drop = FALSE]
  if (!nrow(candidates)) {
    stop("Unknown Monarch provider: ", provider_id, call. = FALSE)
  }
  if (!is.null(release_id)) {
    ...
    if (!nrow(selected)) {
      stop("Unknown Monarch release for provider: ", release_id, call. = FALSE)
    }
    return(selected)
  }
  if (!is.null(as_of)) {
    if (length(as_of) != 1L) {
      stop("`as_of` must be one typed Date or POSIXct value.", call. = FALSE)
    }
    cutoff <- ducksemantics_typed_time(as_of, "as_of")
    times <- ducksemantics_typed_time(
      candidates$effective_date,
      "releases$effective_date",
      allow_na = TRUE
    )
    selected <- candidates[!is.na(times) & times <= cutoff, , drop = FALSE]
    if (!nrow(selected)) {
      stop("No Monarch release is visible at `as_of`.", call. = FALSE)
    }
    times <- ducksemantics_typed_time(
    ...
    return(selected[
      order(times, selected$release_id, decreasing = TRUE)[[1L]],
      ,
      drop = FALSE
    ])
  }
  if (
    !is.numeric(as_of_ordinal) ||
      length(as_of_ordinal) != 1L ||
      is.na(as_of_ordinal) ||
      !is.finite(as_of_ordinal)
  ) {
    stop("`as_of_ordinal` must be one finite numeric scalar.", call. = FALSE)
  }
  selected <- candidates[
    !is.na(candidates$source_ordinal) &
      candidates$source_ordinal <= as_of_ordinal,
    ,
    drop = FALSE
  ]
  if (!nrow(selected)) {
    stop("No Monarch release is visible at `as_of_ordinal`.", call. = FALSE)
  }
  selected[
    order(selected$source_ordinal, selected$release_id, decreasing = TRUE)[[
      1L
    ]],
    ,
    drop = FALSE
...
ducksemantics_literature_snapshot_catalog <- function(snapshots, conn = NULL) {
  ...
  snapshots <- ducksemantics_require_data_frame(
    snapshots,
    "snapshots",
    c("provider_id", "snapshot_id", "high_water_ordinal")
  )
  if (!"effective_at" %in% names(snapshots)) {
    snapshots$effective_at <- as.POSIXct(
      rep(NA_real_, nrow(snapshots)),
      origin = "1970-01-01",
      tz = "UTC"
    )
  }
  snapshots <- snapshots[,
    c("provider_id", "snapshot_id", "high_water_ordinal", "effective_at"),
    drop = FALSE
  ]
  ducksemantics_validate_required_text(
    snapshots,
    c("provider_id", "snapshot_id"),
    "snapshots"
  )
  snapshots$high_water_ordinal <- ducksemantics_literature_ordinal(
    snapshots$high_water_ordinal,
    "snapshots$high_water_ordinal"
  )
  ducksemantics_literature_optional_timestamp(
    snapshots$effective_at,
    "snapshots$effective_at"
  )
  if (anyDuplicated(snapshots[c("provider_id", "snapshot_id")])) {
    stop(
      "`provider_id` and `snapshot_id` must uniquely identify snapshots.",
      call. = FALSE
    )
  }
  snapshots
...
#' @export
ducksemantics_literature_retrieve <- function(
  articles,
  sections,
  snapshots,
  query,
  ...
  articles <- ducksemantics_literature_articles(ducksemantics_collect_relation(
    articles,
    conn,
    "articles"
  ))
  sections <- ducksemantics_literature_sections(ducksemantics_collect_relation(
    sections,
    conn,
    "sections"
  ))
  ...
  if (
    !is.null(section_names) &&
      (!is.character(section_names) ||
        !length(section_names) ||
        anyNA(section_names) ||
        !all(nzchar(section_names)))
  ) {
    stop(
      "`section_names` must be NULL or non-empty character names.",
      call. = FALSE
    )
  }
  snapshot <- snapshots[
    snapshots$provider_id == provider_id & snapshots$snapshot_id == snapshot_id,
    ,
    drop = FALSE
  ]
  if (!nrow(snapshot)) {
    stop(
      "Unknown literature snapshot for provider: ",
      snapshot_id,
      call. = FALSE
    )
  }
  ...
  visible <- ducksemantics_literature_visible_articles(
    articles,
    high_water_ordinal
  )
  if (!nrow(visible)) {
    return(ducksemantics_empty_literature_hits(snapshot))
  }
  section_key <- ducksemantics_literature_version_key(sections)
  visible_key <- ducksemantics_literature_version_key(visible)
  matched <- sections[
    section_key %in% visible_key & sections$provider_id == provider_id,
    ,
    drop = FALSE
  ]
  if (!is.null(section_names)) {
    matched <- matched[matched$section %in% section_names, , drop = FALSE]
  }
  if (!nrow(matched)) {
    return(ducksemantics_empty_literature_hits(snapshot))
  }
  matches <- lapply(seq_len(nrow(matched)), function(i) {
    ...
    data.frame(
      provider_id = rep(provider_id, length(starts)),
      snapshot_id = rep(snapshot_id, length(starts)),
      high_water_ordinal = rep(high_water_ordinal, length(starts)),
      effective_at = snapshot$effective_at[rep.int(1L, length(starts))],
      article_id = rep(matched$article_id[[i]], length(starts)),
      pmid = rep(matched$pmid[[i]], length(starts)),
      version_id = rep(matched$version_id[[i]], length(starts)),
      source_ordinal = rep(matched$source_ordinal[[i]], length(starts)),
      section = rep(matched$section[[i]], length(starts)),
      start_offset = as.integer(starts - 1L),
      end_offset = as.integer(starts + widths - 1L),
      source_text = substring(matched$text[[i]], starts, starts + widths - 1L),
      section_text = rep(matched$text[[i]], length(starts)),
      stringsAsFactors = FALSE
    )
  })
  out <- ducksemantics_bind_or_empty(
    matches,
    ducksemantics_empty_literature_hits(snapshot)
  )
  if (nrow(out)) {
    ...
    if (!all(nzchar(out$source_text))) {
      stop("Literature retrieval produced an empty source span.", call. = FALSE)
    }
  }
...
ducksemantics_literature_articles <- function(articles) {
  ...
  ducksemantics_validate_required_text(
    articles,
    c("provider_id", "article_id", "pmid", "version_id"),
    "articles"
  )
  articles$source_ordinal <- ducksemantics_literature_ordinal(
    articles$source_ordinal,
    "articles$source_ordinal"
  )
  if (!is.logical(articles$is_deleted) || anyNA(articles$is_deleted)) {
    stop(
      "`articles$is_deleted` must contain logical values without NA.",
      call. = FALSE
    )
  }
  ...
  if (
    anyDuplicated(articles[c("provider_id", "article_id", "source_ordinal")])
  ) {
    stop(
      "Article provider/id/source-ordinal rows must be unique.",
      call. = FALSE
    )
  }
  articles
...
ducksemantics_literature_sections <- function(sections) {
  ...
  ducksemantics_validate_required_text(
    sections,
    c("provider_id", "article_id", "pmid", "version_id", "section", "text"),
    "sections"
  )
  sections$source_ordinal <- ducksemantics_literature_ordinal(
    sections$source_ordinal,
    "sections$source_ordinal"
  )
  sections
}

ducksemantics_literature_validate_section_versions <- function(
  articles,
  sections
) {
  ...
  if (length(setdiff(unique(section_key), article_key))) {
    stop(
      "Every literature section must bind a supplied provider/article/PMID/version/source-ordinal row.",
      call. = FALSE
    )
  }
  invisible(TRUE)
...
ducksemantics_literature_visible_articles <- function(
  articles,
  high_water_ordinal
) {
  if (!nrow(articles)) {
  ...
  eligible <- articles[
    articles$source_ordinal <= high_water_ordinal,
    ,
    drop = FALSE
  ]
  if (!nrow(eligible)) {
  ...
  eligible <- eligible[
    order(eligible$provider_id, eligible$article_id, -eligible$source_ordinal),
    ,
    drop = FALSE
  ]
  latest <- eligible[
    !duplicated(eligible[c("provider_id", "article_id")]),
    ,
    drop = FALSE
  ]
...
ducksemantics_empty_literature_hits <- function(snapshot) {
  data.frame(
    ...
    high_water_ordinal = numeric(),
    effective_at = snapshot$effective_at[FALSE],
    article_id = character(),
    pmid = character(),
    version_id = character(),
    source_ordinal = numeric(),
    section = character(),
...
ducksemantics_literature_ordinal <- function(x, arg) {
  if (!is.numeric(x) || anyNA(x) || !all(is.finite(x)) || any(x != trunc(x))) {
    stop(
      "`",
      arg,
      "` must contain finite integer values without NA.",
      call. = FALSE
    )
  }
  as.numeric(x)
...
#' @export
ducksemantics_monarch_project_pack <- function(
  conn,
  source_catalog,
  releases,
  provider_id,
  ...
  ducksemantics_require_connection(conn)
  source_catalog <- ducksemantics_require_identifier(
    source_catalog,
    "source_catalog"
  )
  ...
  if (identical(tolower(release_id), "latest")) {
    stop(
      "`release_id` must name one exact dated release; `latest` is not allowed.",
      call. = FALSE
    )
  }
  source_catalog <- ducksemantics_monarch_pack_attached_catalog(
    conn,
    source_catalog
  )
  catalog <- ducksemantics_monarch_release_catalog(releases, conn)
  selected <- catalog[
    catalog$provider_id == provider_id & catalog$release_id == release_id,
    ,
    drop = FALSE
  ]
  if (nrow(selected) != 1L) {
    stop(
      "`provider_id` and `release_id` must identify exactly one typed Monarch release catalog row.",
      call. = FALSE
    )
  }
  ...
  DBI::dbWithTransaction(conn, {
    ...
    relations <- list(
      gene_disease = c(
        first = "gene",
        first_category = "biolink:Gene",
        second = "disease",
        second_category = "biolink:Disease"
      ),
      disease_phenotype = c(
        first = "disease",
        first_category = "biolink:Disease",
        second = "phenotype",
        second_category = "biolink:PhenotypicFeature"
      ),
      gene_phenotype = c(
        first = "gene",
        first_category = "biolink:Gene",
    ...
    for (relation in names(relations)) {
      ...
      DBI::dbExecute(
        conn,
        ducksemantics_monarch_pack_role_view_sql(
          conn,
          views[["associations"]],
          views[[relation]],
          spec[["first"]],
  ...
  data.frame(
    relation = names(views),
    view_name = unname(views),
...
ducksemantics_monarch_pack_attached_catalog <- function(conn, source_catalog) {
  ...
  matching <- databases[
    tolower(databases$database_name) == tolower(source_catalog),
    ,
    drop = FALSE
  ]
  if (
    nrow(matching) != 1L ||
      identical(tolower(source_catalog), tolower(current)) ||
...
ducksemantics_monarch_pack_validate_source <- function(conn, source_catalog) {
  ...
  expected_types <- c(
    denormalized_edges = "VIEW",
    nodes = "BASE TABLE",
    information_content = "BASE TABLE",
    node_has_phenotype = "BASE TABLE"
  )
...
ducksemantics_monarch_pack_validate_table <- function(
  ...
  types <- stats::setNames(
    toupper(table_columns$data_type),
    table_columns$column_name
  )
  invalid_text <- text[
    !vapply(types[text], ducksemantics_monarch_pack_is_text, logical(1))
  ]
  invalid_lists <- text_list[
    !vapply(
      types[text_list],
      ducksemantics_monarch_pack_is_text_list,
      logical(1)
    )
  ]
  invalid_numeric <- numeric[
    !vapply(types[numeric], ducksemantics_monarch_pack_is_numeric, logical(1))
  ]
...
ducksemantics_monarch_pack_is_text_list <- function(type) {
  is.character(type) &&
    length(type) == 1L &&
    !is.na(type) &&
    grepl("^(VARCHAR|TEXT|STRING)\\[\\]$", type)
}
...
ducksemantics_monarch_pack_is_numeric <- function(type) {
  is.character(type) &&
    ...
    !is.na(type) &&
    grepl(
      "^(U?TINYINT|U?SMALLINT|U?INTEGER|U?BIGINT|HUGEINT|UHUGEINT|FLOAT|DOUBLE|DECIMAL.*)$",
      type
    )
}

ducksemantics_monarch_pack_role_view_sql <- function(
  conn,
  association_view,
  view,
  first,
  ...
  subject_first <- ducksemantics_monarch_pack_category_sql(
    conn,
    "subject_category",
    first_category
  )
  object_first <- ducksemantics_monarch_pack_category_sql(
    conn,
    "object_category",
    first_category
  )
  subject_second <- ducksemantics_monarch_pack_category_sql(
    conn,
    "subject_category",
    second_category
  )
  object_second <- ducksemantics_monarch_pack_category_sql(
    conn,
    "object_category",
    second_category
  )
  subject_missing <- ducksemantics_monarch_pack_missing_category_sql(
    conn,
    "subject_category"
  )
  object_missing <- ducksemantics_monarch_pack_missing_category_sql(
    conn,
    "object_category"
  )
  subject_malformed <- ducksemantics_monarch_pack_malformed_category_sql(
    conn,
    "subject_category"
  )
  object_malformed <- ducksemantics_monarch_pack_malformed_category_sql(
    conn,
    "object_category"
  )
  role_columns <- DBI::SQL(paste(
    c(
      ducksemantics_monarch_pack_role_columns_sql(
        conn,
        first,
        "subject",
        "object"
...
ducksemantics_monarch_pack_role_columns_sql <- function(
  conn,
  role,
  forward,
  reverse
  ...
  vapply(
    names(suffixes),
    function(suffix) {
      forward_column <- if (identical(suffix, "id")) {
        forward
      } else {
        paste0(forward, "_", suffixes[[suffix]])
      }
      reverse_column <- if (identical(suffix, "id")) {
        reverse
      } else {
        paste0(reverse, "_", suffixes[[suffix]])
      }
...
ducksemantics_typed_time <- function(x, arg, allow_na = FALSE) {
  ...
  if (
    !allow_na && (length(x) != 1L && anyNA(x) || length(x) == 1L && is.na(x))
  ) {
    stop("`", arg, "` must not contain NA.", call. = FALSE)
  }
  numeric <- as.numeric(x)
  if (any(!is.na(numeric) & !is.finite(numeric))) {
    stop("`", arg, "` must contain finite times.", call. = FALSE)
  }
  numeric
}
