...
ducksemantics_require_data_frame <- function(
  x,
  arg,
  required = character(),
  allow_empty = TRUE
...
ducksemantics_require_matrix <- function(x, arg, rows = NULL) {
  ...
  if (!nrow(x) || !ncol(x) || anyNA(x) || !all(is.finite(x))) {
    stop(
      "`",
      arg,
...
#' @export
ducksemantics_embedding_batch <- function(
  embeddings,
  subject_id,
  subject_kind = "node",
  provider = "embedding",
  ...
  embeddings <- ducksemantics_require_matrix(
    embeddings,
    "embeddings",
    length(subject_id)
  )
  ...
  if (anyNA(subject_id) || !all(nzchar(subject_id))) {
    stop(
      "`subject_id` must contain one non-empty value per embedding row.",
      call. = FALSE
    )
  }
  n <- nrow(embeddings)
  if (!is.null(text) && !(length(text) %in% c(1L, n))) {
    stop(
      "`text` must be NULL, length 1, or one value per embedding row.",
      call. = FALSE
    )
  }
  if (!is.null(attrs) && !(length(attrs) %in% c(1L, n))) {
    stop(
      "`attrs` must be NULL, length 1, or one value per embedding row.",
      call. = FALSE
    )
  }
  out <- data.frame(
    ...
    text = if (is.null(text)) {
      ...
      rep(as.character(text), length.out = n)
    },
    dim = rep(as.integer(ncol(embeddings)), n),
    attrs = if (is.null(attrs)) {
      NA_character_
    } else {
      rep(as.character(attrs), length.out = n)
    },
...
#' @export
ducksemantics_token_embedding_batch <- function(
  embeddings,
  subject_id,
  subject_kind = "node",
  provider = "embedding",
  ...
  if (anyNA(subject_id) || !all(nzchar(subject_id))) {
    stop(
      "`subject_id` must contain one non-empty value per token row.",
      call. = FALSE
    )
  }
  if (is.null(block_id)) {
    block_id <- ducksemantics_default_block_id(
      provider,
      subject_kind,
      subject_id
    )
  ...
  if (length(block_id) != n || anyNA(block_id) || !all(nzchar(block_id))) {
    stop(
      "`block_id` must contain one non-empty value per token row.",
      call. = FALSE
    )
  }
  ...
  if (
    !is.numeric(token_index) ||
      length(token_index) != n ||
      anyNA(token_index) ||
      !all(is.finite(token_index)) ||
      any(token_index < 0) ||
      any(token_index != floor(token_index))
  ) {
    stop(
      "`token_index` must contain one non-negative integer per token row.",
      call. = FALSE
    )
  }
  if (anyDuplicated(data.frame(block_id, subject_id, token_index))) {
    stop(
      "`token_index` must be unique within each block and subject.",
      call. = FALSE
    )
  }
  recycle_optional <- function(x, arg) {
    ...
    if (!(length(x) %in% c(1L, n))) {
      stop(
        "`",
        arg,
        "` must be NULL, length 1, or one value per token row.",
        call. = FALSE
      )
    }
    rep(x, length.out = n)
  ...
  if (
    any(
      !is.na(start_offset) &
        (!is.finite(start_offset) |
          start_offset < 0 |
          start_offset != floor(start_offset))
    ) ||
      any(
        !is.na(end_offset) &
          (!is.finite(end_offset) |
            end_offset < 0 |
            end_offset != floor(end_offset))
      ) ||
      any(xor(is.na(start_offset), is.na(end_offset))) ||
      any(!is.na(start_offset) & start_offset >= end_offset)
  ) {
    stop(
      "Token source offsets must be paired, non-negative, and non-empty half-open spans.",
      call. = FALSE
    )
  }
  out <- data.frame(
...
#' @export
ducksemantics_token_embedding_batch_from_provider <- function(
  text,
  provider,
  subject_id = text,
  subject_kind = "node",
  ...
  subject_id <- as.character(subject_id)
  if (
    length(subject_id) != length(text) ||
      anyNA(subject_id) ||
      !all(nzchar(subject_id))
  ) {
    stop(
      "`subject_id` must contain one non-empty value per input text.",
      call. = FALSE
    )
  }
  ducksemantics_require_text(subject_kind, "subject_kind")
  ...
  rows <- lapply(seq_along(tokens), function(i) {
    one <- tokens[[i]]
    if (is.null(one$embeddings)) {
      stop(
        "Token embedding object ",
        i,
        " must contain `embeddings`.",
        call. = FALSE
      )
    }
    ...
    ducksemantics_token_embedding_batch(
      ...
      subject_id = rep(subject_id[[i]], n),
      subject_kind = subject_kind,
      provider = provider_label,
      token_index = one$token_index %||% (seq_len(n) - 1L),
      block_id = if (is.null(block_id)) {
        rep(
          ducksemantics_default_block_id(
            provider_label,
            subject_kind,
            subject_id[[i]]
          ),
