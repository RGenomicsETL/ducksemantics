...
#' @export
ducksemantics_token_embedding_provider <- function(
  fun,
  ...
  ducksemantics_require_text(label, "label")
  ducksemantics_function_token_embedding_provider_class(
    fun = fun,
    label = label
  )
}
...
#' @return An object implementing [DucksemanticsEmbeddingProvider].
#' @export
ducksemantics_embeddinggemma_provider <- function(
  model,
  ...
  if (
    ...
    stop("`task` must be an EmbeddingGemma task.", call. = FALSE)
  }
  if (!is.null(title) && !identical(task, "retrieval_document")) {
    stop("`title` is valid only for retrieval_document.", call. = FALSE)
  }
  if (!dimensions %in% c(768, 512, 256, 128)) {
    stop("`dimensions` must be 768, 512, 256, or 128.", call. = FALSE)
  }
  ducksemantics_embeddinggemma_provider_class(
    ...
    normalize = ducksemantics_require_flag(normalize, "normalize"),
    truncate = ducksemantics_require_flag(truncate, "truncate"),
    check_interrupt = ducksemantics_require_flag(
      check_interrupt,
      "check_interrupt"
    )
  )
...
#' @return An object implementing [DucksemanticsTokenEmbeddingProvider].
#' @export
ducksemantics_colbert_provider <- function(
  model,
...
ducksemantics_bebel_tool_judgment_parser <- function(tool_name = NULL) {
  if (
    !is.null(tool_name) &&
      (!is.character(tool_name) || anyNA(tool_name) || !all(nzchar(tool_name)))
  ) {
    stop(
      "`tool_name` must be NULL or non-empty character names.",
      call. = FALSE
    )
  }
...
S7::method(
  ducksemantics_run,
  ducksemantics_function_prompt_runner_class
) <- function(provider, prompt, ...) {
  ducksemantics_response_text(provider@fun(
    ducksemantics_require_text(prompt, "prompt"),
    ...
  ))
}
S7::method(ducksemantics_run, ducksemantics_bebel_runner_class) <- function(
  provider,
  prompt,
  ...
) {
  ...
  Rbebelm::bebel_append_user(
    provider@agent,
    ducksemantics_require_text(prompt, "prompt")
  )
  ducksemantics_response_text(Rbebelm::bebel_assistant_turn(
    provider@agent,
    on_event = provider@on_event
  ))
}
S7::method(
  ducksemantics_embed,
  ducksemantics_function_embedding_provider_class
) <- function(provider, text, ...) {
  if (!is.character(text) || anyNA(text)) {
    stop("`text` must be a character vector without NA.", call. = FALSE)
  }
  ducksemantics_require_matrix(
    provider@fun(text, ...),
    "provider result",
    length(text)
  )
...
S7::method(
  ...
  if (!is.list(out) || length(out) != length(text)) {
    stop(
      "Token embedding providers must return one object per input text.",
      call. = FALSE
    )
  }
  out
...
S7::method(
  ...
  if (!is.character(text) || anyNA(text) || !all(nzchar(text))) {
    stop(
      "`text` must be a non-empty character vector without NA.",
      call. = FALSE
    )
  }
...
S7::method(
  ...
  parsed <- ducksemantics_normalize_judgment_payload(ducksemantics_parse_json_response(
    response
  ))
...
S7::method(
  ducksemantics_parse,
  ducksemantics_bebel_tool_judgment_parser_class
) <- function(parser, response, ...) {
  if (!requireNamespace("Rbebelm", quietly = TRUE)) {
  ...
  if (!is.null(parser@tool_name) && length(calls)) {
    calls <- calls[vapply(
      calls,
      function(x) x$name %in% parser@tool_name,
      logical(1)
    )]
  }
  if (length(calls)) {
    return(ducksemantics_lists_to_data_frame(lapply(calls, `[[`, "arguments")))
  }
...
ducksemantics_bebel_tool_blocks <- function(response) {
  ...
  pieces <- strsplit(response, "<\\|tool_call_start\\|>", perl = TRUE)[[1L]][
    -1L
  ]
...
ducksemantics_parse_json_candidates <- function(candidates) {
  for (candidate in candidates[nzchar(trimws(candidates))]) {
    parsed <- try(
      ducksemantics_normalize_judgment_payload(ducksemantics_parse_json_response(
        candidate
      )),
      silent = TRUE
    )
    if (!inherits(parsed, "try-error")) {
      return(ducksemantics_require_data_frame(parsed, "parsed"))
    }
  }
  stop(
    "BebeLM response did not contain judgment tool calls or JSON.",
    call. = FALSE
  )
}
